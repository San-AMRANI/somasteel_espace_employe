...
class AttendanceController extends Controller
{
    public function index(Request $request)
    {
        ...
        if (Auth::user()->isRH()) {
            $users = User::where('projet', 'LAMINOIR')
                ->get(['id', 'matricule', 'nom', 'prénom', 'shift_id', 'service']);
            $allUsers = User::with('shift')
                ->where('projet', 'LAMINOIR')
                ->where('projet', 'ACIERIE')
                ->get();
        } else {
            $users = User::where('projet', 'LAMINOIR')->where('responsable_hiarchique', Auth::user()->matricule)
                ->get(['id', 'matricule', 'nom', 'prénom', 'shift_id', 'service']);
            $allUsers = User::with('shift')
                ->where('projet', 'LAMINOIR')->where('responsable_hiarchique', Auth::user()->matricule)
                ->get();
        }
        ...
        return view('absence.absenceDec', compact('users', 'shifts', 'résultatData', 'attendances', 'allUsers', 'today', 'declaredAttendances', 'uniqueServices'));
    }
    ...
    public function export(Request $request)
    {
        ...
        return Excel::download(new ShiftsExport($exportData->toArray(), $date), 'shifts.xlsx');
    }
